...
def split_train_test(samples:list, percentage_train:float=0.9, limit:int=20000)->Tuple[list,list]:
    random.shuffle(samples)
    samples = samples[:limit]
    split_index = int(len(samples) * percentage_train)
    return samples[:split_index], samples[split_index:]

def create_folders(train_folder:Path, test_folder:Path, train_samples:list, test_samples:list):
...
if __name__ == "__main__":
    ...
    # Get all the samples
    train_samples, test_samples = split_train_test(list(args.dataset.glob("*.npz")), args.percentage, args.limit)
